...
struct key_type key_map[12*3*4] =
{
  //level 0
...
struct key_type get_key_left(int row_pin, int col_pin, int fn_l){
...
void set_usb(struct key_type k){
...
void loop() {
  ...
  for (;row_pin<=ROW_PIN_END;++row_pin){
    ...
    for (;col_pin<=COL_PIN_END;++col_pin){
      ...
      struct key_type k = get_key_left(row_pin, col_pin, fn_l);
